    ...
    func nextQuestion(){
        ...
        if questionNumber < nrQuestionBad.count {
         ...
         //    progressLabel.text = "Pytanie \(nrQuestionRandom[questionNumber]) / 10"
            ...
            buttonBadAnswer.setImage(UIImage(named: "raven/test-de-raven-\(String(nrQuestionBad[questionNumber]))_\(abc[badAnswer[questionNumber]])"), for: .normal)
          ...
          buttonRightAnswer.setImage(UIImage(named: "raven/test-de-raven-\(String(nrQuestionBad[questionNumber]))_\(abc[correctAnswer])"), for: .normal)
